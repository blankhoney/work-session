---
name: work-session
description: Explicit long-running implementation orchestration with durable recovery, bounded workers, and fresh quality gates.
user-invocable: true
...
3. Start or recover one persistent idle **State Keeper**, sole current writer of the state file and exact managed blocks. When `ActiveSession` exists, automatically resume or conditionally claim it by `runtime.md`; never ask the user to repeat invocation or determine process liveness. In root `CLAUDE.md` the Keeper replaces every exact old/current Work Session block with one runtime block from `references/agent.md` and one locator-only block, preserving all unmarked content:
...
- **Fresh worker:** one bounded construct/integrate/correct, simplify, review, or initial test mode; self-contained brief; no children; one writer per path.
- **Final Tester:** one fresh black-box session launched only after the runnable target is ready; keep it only for failed/affected-item reruns inside the same final QA lifecycle, then stop it.
...
2. Construct under `Q-WRITE`. Each completed non-trivial `WorkItem` gets exactly one fresh `Q-SIMPLIFY`; mechanically trivial text/comment/format work skips it. Dispatch it and keep building: construction on stable interfaces and non-overlapping paths never waits for a simplify, consultation, or review to return.
...
4. Dispatch fresh independent `Q-REVIEW` as an advisory parallel check while preparing the final runnable target. It cannot return work, block or delay delivery checks, change active construction state or direction, or trigger repair. Candidate drift only stales that report.
...
`quality.md` alone defines quality rules. Do not require or duplicate a PRD/design/Gherkin lifecycle; task sources own intent and acceptance. No retrieval role, recursive audit, quality manager, hidden fallback, automatic companion management, hooks, scripts, executables, bootstrap, or preflight. A commit, push, home installation, or external publication explicitly named in the accepted task source remains authorized through recovery and must not be requested again; when absent, each requires a separate explicit user command. Never rewrite Git history. All artifacts obey `runtime.md` no-secrets rules.
