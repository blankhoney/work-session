...
Decide from goal, project rules/plan, code, git/tasks, and artifacts. Retrieval is conditional: dispatch `luna-investigator` for one closed named-path fact or `luna-repository-explorer` for bounded discovery under one absolute root only when a construction fact/assertion is missing; history/self-reports are clues.
...
Only Leader revises the contract from current evidence. Teammates/Sidecar cannot add requirements, tests, contracts, schemas, architecture, or completion conditions.

## Brief and dispatch

Use fixed user-level Agent types directly and never override their model/effort or use `fork`: `work-session-constructor` for Sol-low construction/correction; fresh one-shot `work-session-simplifier`, `work-session-reviewer`, and `work-session-tester` for Sol-high simplify, original-task audit, and QA; one named persistent `work-session-sidecar` Mate for Sol-high Sidecar. Retrieval uses the existing Luna-xhigh Agents above. No worker creates children. A constructor Mate may be resumed only with unchanged contract/source identity; simplifier/reviewer/tester are never resumed or reused. A reviewer is optional only when the original task requires that bounded audit and never creates another gate.
...
For each Unit, prove relevant assertions, mark `construction-complete`, record `SimplifyBaseIdentity` for exact scope, and start one fresh asynchronous `work-session-simplifier` with a self-contained brief and no parent/prior context. Stable-interface calls/reads and non-overlap construction never wait.
...
After integration cutoff, Sidecar, and any correction, reread the task-source files; then launch a fresh `work-session-tester` with a self-contained brief and no construction/prior context to run exact original `QAObject`/`FinalAcceptance` once. Add no checks. A failed item may receive one bounded constructor repair and only that item is rerun by another fresh tester. Leader may mechanically summarize bounded paths/checks/status/blockers without mandatory validator/summarizer or invented conclusions.
